<!-- Accept Modal -->
<form method="POST" action="{{ action('Voyager\DepositController@accept') }}">
  @csrf
...
<!-- Refuse Modal -->
<form method="POST" action="{{ action('Voyager\DepositController@refuse') }}">
@csrf
<div class="modal fade" id="refuse-{{ $data->id }}" tabindex="-1" role="dialog" aria-labelledby="refuse_{{ $data->id }}_label" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        Do you want to refuse this deposit ?
      </div>
      <div class="modal-footer">
		<input type="hidden" name="id" value="{{ $data->id }}">
